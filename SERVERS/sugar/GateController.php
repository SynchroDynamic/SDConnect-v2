...
class GateController {
    function route($pSplit, $pathSplit) {switch ($pSplit[2]) {case "Update":return \sugar\DataService::update($pathSplit);
            case "Get":$col = empty($pathSplit[0]) ? null : $pathSplit[0];$op = empty($pathSplit[1]) ? null : $pathSplit[1];
                ...
                return json_encode(\sugar\DataService::getRecords($whereCol, $whereOperator, $where));
            case "Delete":
                return json_encode(\sugar\DataService::delete($pathSplit[0]));
            default: return "";
        }
    }
}