...
namespace sugar {
    class DAO {
        ...
        function __construct() { $this->date = date("Y-m-d H:i:s"); $this->conn = new \ServerDatabase(); $this->fullCols = array('id','uid','salad','frozen'); $this->columns = array('salad','frozen'); $this->tableName = "sugar";}
        ...
        public function retrieve($whereCol, $whereOperator, $where) { $this->conn->getConnection(); if ($whereCol == null) {
            ...
            $sugarArray = array(); $count = 0; while ($row = $records->fetch(\PDO::FETCH_ASSOC)) { extract($row); $r = array("id" => $id,"uid" => $uid,"salad" => $salad,"frozen" => $frozen); $sugarArray[$count++] = $r; }return $sugarArray; }
