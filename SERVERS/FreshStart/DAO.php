...
namespace FreshStart {
    class DAO {
        ...
        function __construct() { $this->date = date("Y-m-d H:i:s"); $this->conn = new \ServerDatabase(); $this->fullCols = array('id','name'); $this->columns = array('name'); $this->tableName = "FreshStart";}
        ...
        public function retrieve($whereCol, $whereOperator, $where) { $this->conn->getConnection(); if ($whereCol == null) {
            ...
            $FreshStartArray = array(); $count = 0; while ($row = $records->fetch(\PDO::FETCH_ASSOC)) { extract($row); $r = array("id" => $id,"name" => $name); $FreshStartArray[$count++] = $r; }return $FreshStartArray; }
