...
namespace test {
    class DAO {
        ...
        function __construct() { $this->date = date("Y-m-d H:i:s"); $this->conn = new \ServerDatabase(); $this->fullCols = array('id','test'); $this->columns = array('test'); $this->tableName = "test";}
        ...
        public function retrieve($whereCol, $whereOperator, $where) { $this->conn->getConnection(); if ($whereCol == null) {
            ...
            $testArray = array(); $count = 0; while ($row = $records->fetch(\PDO::FETCH_ASSOC)) { extract($row); $r = array("id" => $id,"test" => $test); $testArray[$count++] = $r; }return $testArray; }
