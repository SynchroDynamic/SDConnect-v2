<?php
namespace flippyflappy {
    class DAO {
        private $fullCols, $columns, $tableName, $conn, $date;
        function __construct() { $this->date = date("Y-m-d H:i:s"); $this->conn = new \ServerDatabase(); $this->fullCols = array('id','free','see','dee'); $this->columns = array('free','see','dee'); $this->tableName = "flippyflappy";}
        ...
        public function retrieve($whereCol, $whereOperator, $where) { $this->conn->getConnection(); if ($whereCol == null) {
            ...
            $flippyflappyArray = array(); $count = 0; while ($row = $records->fetch(\PDO::FETCH_ASSOC)) { extract($row); $r = array("id" => $id,"free" => $free,"see" => $see,"dee" => $dee); $flippyflappyArray[$count++] = $r; }return $flippyflappyArray; }
